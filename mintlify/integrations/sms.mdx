...
<Note>
  SMS behavior depends on your plan, add-ons, compliance state, and sender configuration.
</Note>
...
AI SMS lets the receptionist respond to inbound text conversations using the same business context used for calls. The add-on is available on Pro when checkout is configured, and it requires SMS compliance setup before hosted business-number sending is ready.
...
Until hosted SMS compliance is approved, routing may stay on a shared alert sender and AI SMS may remain blocked.
