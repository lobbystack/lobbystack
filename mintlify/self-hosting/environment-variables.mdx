...
| `DASHBOARD_TEST_CALL_TOKEN` | Convex, voice gateway | Optional shared token for elevated dashboard test-call rate limits outside development. Use the same value in both services. |
...
| `VITE_WEB_CALL_ENDPOINT` | Voice gateway URL for in-dashboard browser test calls. Defaults to production voice gateway when unset. |
...
| `DASHBOARD_TEST_CALL_TOKEN` | Optional shared token sent by the gateway to Convex for dashboard test calls. Must match the Convex value when configured. |
