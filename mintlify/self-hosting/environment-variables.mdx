...
| `CONVEX_CLOUD_ORIGIN` | Convex backend container | Must match the public or smoke-test Convex client URL. |
| `CONVEX_SITE_ORIGIN` | Convex backend container | Must match the public or smoke-test Convex HTTP actions URL. |
...
<Note>
  After changing public URLs (`CONVEX_URL`, `CONVEX_SITE_URL`, `APP_BASE_URL`, hostnames), rebuild the web image and rerun `pnpm self-hosted:convex:env`. See [Docker Compose](/self-hosting/docker-compose#production-go-live-checklist).
</Note>
