...
export function AffiliatePage() {
  ...
  const ensureAffiliateProfile = useObservedMutation(api.affiliates.activate);
  ...
  useEffect(() => {
    if (summary === undefined || summary.profile || hasRequestedProfile.current) {
      return;
    }

    hasRequestedProfile.current = true;
    void ensureAffiliateProfile({}).catch(() => {
      hasRequestedProfile.current = false;
      toast.error(t("toast.profileFailed"));
    });
  }, [ensureAffiliateProfile, summary, t]);
