...
test("setup skips persist concurrently and require tenant administration", async ({ browser, baseURL }) => {
  ...
  try {
    ...
    expect((await reloaded.json()).steps.filter((step: { status: string }) => step.status === "skipped")).toHaveLength(3);
    expect((await context.request.patch(url, { headers, data: { stepId: "invalid", skipped: true } })).status()).toBe(400);
    expect((await context.request.patch(url, { headers, data: { stepId: "sources", skipped: false } })).status()).toBe(200);
    const unskipped = await context.request.get(url);
    expect((await unskipped.json()).steps.find((step: { id: string }) => step.id === "sources").status).toBe("needs setup");
    ...
    expect((await context.request.patch(url, { headers, data: { stepId: "sources", skipped: true } })).status()).toBe(403);
  } finally {
    await context.close();
