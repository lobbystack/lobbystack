...
vi.mock("next/navigation", () => ({ useRouter: () => ({ push: navigateMock }) }));
const clients: QueryClient[] = [];
afterEach(() => { cleanup(); clients.forEach(client => client.clear()); clients.length = 0; vi.unstubAllGlobals(); });
function OnboardingVerifyPhonePage({ currentUserPhone }: { businessId: string; currentUserPhone?: string; onSignOut: () => void }) {
  ...
  client.setQueryData(["phone-verification", "business-1"], { attempt: null, currentUserPhone: currentUserPhone ?? null });
  return <QueryClientProvider client={client}><OnboardingPhoneVerificationSurface /></QueryClientProvider>;
}
...
describe("OnboardingVerifyPhonePage", () => {
  beforeEach(() => {
    ...
    vi.stubGlobal("fetch", async (url: string, init: RequestInit) => {
      ...
      return Response.json(await startPhoneVerificationMock({ businessId: "business-1", phoneE164: body.phoneNumber }));
    });
