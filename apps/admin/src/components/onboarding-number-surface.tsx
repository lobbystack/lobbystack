...
export function OnboardingNumberSurface() {
  ...
  const skip = useMutation({ mutationFn: () => requestJson(`/api/onboarding/phone-numbers/skip?businessId=${encodeURIComponent(business!.businessId)}`, { method: "POST" }), onSuccess: () => router.push(business?.onboardingStage === "complete" ? "/" : "/onboarding/attribution") });
