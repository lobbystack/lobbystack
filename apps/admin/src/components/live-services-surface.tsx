...
export function LiveServicesSurface() {
  ...
  if (businesses.isLoading) return <TableCardSkeleton columns={5} />;
...
function ServiceEditor({ editing, onOpenChange, open, pending, save }: { editing: Service | null; onOpenChange: (open: boolean) => void; open: boolean; pending: boolean; save: (values: ServiceValues) => Promise<void> }) {
  ...
  const titleId = useId(); const durationId = useId(); const descriptionId = useId(); const activeId = useId();
  ...
  return <Dialog onOpenChange={onOpenChange} open={open}><DialogContent className="sm:max-w-md"><DialogHeader><DialogTitle>{t(editing ? "sections.services.editKnowledge" : "sections.services.addKnowledge")}</DialogTitle><DialogDescription>{t(editing ? "sections.services.editKnowledgeDescription" : "sections.services.addKnowledgeDescription")}</DialogDescription></DialogHeader><form className="flex flex-col gap-6" onSubmit={(event) => { event.preventDefault(); if (!pending && values.name.trim() && Number.isInteger(Number(values.durationMinutes)) && Number(values.durationMinutes) > 0) void save(values); }}><FieldGroup><Field><FieldContent><FieldLabel htmlFor={titleId}>{t("sections.services.fields.title.label")}</FieldLabel><FieldDescription>{t("sections.services.fields.title.hint")}</FieldDescription></FieldContent><Input id={titleId} onChange={(event) => setValues((current) => ({ ...current, name: event.target.value }))} placeholder={t("sections.services.fields.title.placeholder")} value={values.name} /></Field><Field><FieldContent><FieldLabel htmlFor={durationId}>{t("sections.services.fields.duration.label")}</FieldLabel><FieldDescription>{t("sections.services.fields.duration.hint")}</FieldDescription></FieldContent><Input id={durationId} inputMode="numeric" min="1" step="1" onChange={(event) => setValues((current) => ({ ...current, durationMinutes: event.target.value }))} placeholder={t("sections.services.fields.duration.placeholder")} type="number" value={values.durationMinutes} /></Field><Field><FieldContent><FieldLabel htmlFor={descriptionId}>{t("sections.services.fields.content.label")}</FieldLabel><FieldDescription>{t("sections.services.fields.content.hint")}</FieldDescription></FieldContent><Textarea className="min-h-40" id={descriptionId} onChange={(event) => setValues((current) => ({ ...current, description: event.target.value }))} placeholder={t("sections.services.fields.content.placeholder")} value={values.description} /></Field><Field orientation="horizontal"><Switch aria-label={t("sections.services.fields.active.label")} checked={values.active} id={activeId} onCheckedChange={(active) => setValues((current) => ({ ...current, active }))} /><FieldContent><FieldLabel htmlFor={activeId}>{t("sections.services.fields.active.label")}</FieldLabel><FieldDescription>{t("sections.services.fields.active.hint")}</FieldDescription></FieldContent></Field></FieldGroup><DialogFooter><Button className="w-full" disabled={pending} type="submit">{pending ? t("actions.saving") : t(editing ? "actions.saveChanges" : "actions.save")}</Button></DialogFooter></form></DialogContent></Dialog>;
}
