  ...
  return <PageSurface title="Prospect demos" description="Prepare isolated, expiring receptionist workspaces before sharing them with a prospect.">
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
      <div className="space-y-4">
        <div className="flex items-center justify-between"><p className="text-sm text-muted-foreground">{demos.length} demo{demos.length === 1 ? "" : "s"}</p><Button variant="ghost" onClick={() => void load()} disabled={loading}><RefreshCw className="size-4" />Refresh</Button></div>
        {loading ? <Card><CardContent className="py-16 text-center text-sm text-muted-foreground">Loading demos...</CardContent></Card> : null}
        {!loading && demos.length === 0 ? <Card><CardContent className="py-16 text-center text-sm text-muted-foreground">No prospect demos have been prepared yet.</CardContent></Card> : null}
        {demos.map((demo) => {
