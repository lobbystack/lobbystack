...
import { appointments, calls, messages, unitEconomicsRollups } from "@lobbystack/db";
...
export async function GET(request: Request) {
  try {
    return NextResponse.json(await withOperatorTransaction(request, async ({ businessId, tx }) => {
      ...
      const [currentCalls, previousCalls, currentAppointments, previousAppointments, currentMessages, previousMessages, duration, economics] = await Promise.all([
        ...
        tx.select().from(unitEconomicsRollups).where(and(eq(unitEconomicsRollups.businessId, businessId), eq(unitEconomicsRollups.monthKey, now.toISOString().slice(0, 7)))).limit(1),
      ]);
      return {
        periodDays: 30,
