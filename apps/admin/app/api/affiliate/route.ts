...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json(await withBusinessTransaction(getAppDatabase().db, { userId: session.user.id, actorType: "operator" }, async (tx) => {
      const profile = (await tx.select({ id: affiliateProfiles.id, referralCode: affiliateProfiles.referralCode, status: affiliateProfiles.status, payoutEmail: affiliateProfiles.payoutEmail }).from(affiliateProfiles).where(eq(affiliateProfiles.userId, session.user.id)).limit(1))[0];
      if (!profile) return { profile: null, stats: null, clicks: [], attributions: [], commissions: [], payouts: [] };
      const siteUrl = (process.env.SITE_URL ?? process.env.NEXT_PUBLIC_SITE_URL ?? "https://lobbystack.com").replace(/\/$/, "");
      ...
      return { profile: { ...profile, referralLink: `${siteUrl}/signup?ref=${encodeURIComponent(profile.referralCode)}` }, stats: stats[0] ?? { clickCount: 0, referralCount: 0, conversionCount: 0, pendingCommissionCents: 0, paidCommissionCents: 0 }, clicks, attributions, commissions, payouts };
    }));
  } catch (error) {
