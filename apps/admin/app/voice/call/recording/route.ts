...
export async function POST(request: Request) {
  try {
    ...
    await persistCallRecording(createWorkerDomainContext(), { businessId, callId, durationMs: Number(new URL(request.url).searchParams.get("durationMs") ?? 0), contentType: request.headers.get("content-type") ?? "audio/wav", body }, createStorageProvider());
