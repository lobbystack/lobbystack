...
import { completeCall, recordProspectDemoCallOutcome } from "@lobbystack/domain";
...
export async function POST(request: Request) {
  try {
    ...
    const body = voiceCallCompleteRequestSchema.parse(JSON.parse(rawBody));
    const resolved = await getAppDatabase().db.execute<{ business_id: string }>(sql`select app.resolve_business_by_call_id(${body.callId}::uuid) as business_id`);
