...
The findings above have now been addressed on this branch:

- SEC-001 remediated:
  - `confirmSelection` is no longer trusted as booking authority.
  - `selectedStartsAt` still resolves the intended slot, but the backend now requires the current SMS to satisfy `looksLikeBookingConfirmation(prompt)` before any booking mutation occurs.
  - Non-confirming prompts now return a pending-confirmation result instead of creating an appointment, even if the model supplies `confirmSelection: true`.
- SEC-002 remediated:
  - appointment-change detection now treats `cancel` / `reschedule` style verbs as strong signals, but requires appointment context for generic verbs like `change` and `move`
  - unrelated prompts such as `Can you change to English?` and `Move me to a human.` no longer disclose appointment details
- SEC-003 remediated:
  - the ambient `bookingStateSummary` no longer includes exact confirmed service/date/time details
  - it now exposes only coarse state and requires exact appointment details to come from `getCurrentAppointment` / `getAppointmentChangeStatus`
...
- targeted regressions in:
  - [packages/testing/src/smsAvailabilityFlow.test.ts](/Users/raphael/Coding/ai-receptionist/packages/testing/src/smsAvailabilityFlow.test.ts)
  - [packages/testing/src/twilioSmsDelivery.test.ts](/Users/raphael/Coding/ai-receptionist/packages/testing/src/twilioSmsDelivery.test.ts)
