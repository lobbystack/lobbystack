...
import { appointments, businessHours, closures, contacts, enqueueOutbox, notifications, services, staff, withBusinessTransaction, type DatabaseTransaction } from "@lobbystack/db";
...
export async function bookAppointment(
  ...
  return await withBusinessTransaction(context.db, { userId: input.userId, businessId: input.businessId, actorType: input.userId ? "operator" : "worker" }, async (tx) => {
    ...
    const existingContacts = await tx.select({ id: contacts.id }).from(contacts).where(and(eq(contacts.businessId, input.businessId), eq(contacts.phone, input.contactPhone))).limit(1);
...
export async function rescheduleAppointmentForCaller(
  context: DomainContext,
  input: { businessId: string; appointmentId: string; callerPhone: string; startsAt: string },
