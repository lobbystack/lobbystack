...
import { businesses, contacts, createDatabaseClient, users } from "@lobbystack/db";
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) execFileSync("pnpm", ["replacement:import", "--", `--input=${sourcePath}`], { cwd: process.cwd(), env: process.env, stdio: "pipe" });
    execFileSync("pnpm", ["replacement:reconciliation", "--", `--source=${sourcePath}`], { cwd: process.cwd(), env: process.env, stdio: "pipe" });
    console.log(JSON.stringify({ importerRerunIdempotent: true, rowCountsReconciled: true, relationshipsReconciled: true, aggregateTotalsReconciled: true, samplesGenerated: true }));
  } finally {
