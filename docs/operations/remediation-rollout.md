...
The images exclude documentation and editor tooling during installation. `.github/build/prepare-image-workspace.mjs` drops the `mintlify` workspace project and the root `next-devtools-mcp` and `railway` development tools because they pull large optional agent binaries and none of them run at runtime. Keep them in the developer manifests. The Dockerfiles install dependencies with lockfile-first `COPY` and no BuildKit cache mounts: Railway's builder requires a cache mount id prefixed with the service id, which cannot be portable across projects, so layer caching carries the build instead. Keep private build source maps for debugging; runtime images omit them. The migrator still uses `tsx` because its emitted imports do not support plain Node execution. No speculative indexes or public Git-history rewrite accompany this release.
